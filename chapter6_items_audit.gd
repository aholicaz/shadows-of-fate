...
func _run() -> void:
	var rows = JSON.parse_string(FileAccess.get_file_as_string("res://output/chapter6_items/manifest.json"))
	...
	for page in range(ceili(rows.size() / 45.0)):
		...
		viewport.get_texture().get_image().save_png("res://output/chapter6_items/godot_preview_%d.png" % (page+1))
	print("CHAPTER6_ART_AUDIT: ", rows.size(), " resources checked, failures=", failures)
