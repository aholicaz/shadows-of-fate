...
## เดิมระบบย่อ/ขยาย "แยกทีละท่า" ให้ท่านั้นสูงเท่า Display Height พอดี
## ผลคือท่าที่ยกอาวุธสูง (Attack) โดนย่อลงทั้งตัว ตัวมอนเลยหดตอนตี
## เปิดช่องนี้ = คิดสเกลจากท่าอ้างอิงท่าเดียว แล้วใช้สเกลนั้นกับทุกท่า
## (อาวุธที่ยกสูงจะพ้นกรอบขึ้นไป — ถูกแล้ว ตัวไม่หด)
...
## จะถอยกลับไปย่อแยกทีละท่าแบบเดิม ไม่งั้นตัวจะบวมทั้งตัว
@export_range(1.0, 3.0, 0.05) var fit_max_overshoot: float = 1.35

# ---------- ★ รอบ 87 — กรอบโดนตีกว้างเท่าตัวที่วาดจริง ★ ----------
## เดิมกรอบที่ผู้เล่นฟันโดนใช้แค่ Hitbox Size (กล่องชนพื้น) ซึ่งแคบกว่าตัวมอนมาก
## มอนตัวยาว ๆ (อสูรสายฟ้ากว้างบนจอ ~700 px แต่กรอบโดนตีแค่ 105) เลยยืนติดตัวแล้วฟันไม่โดน
## เปิดช่องนี้ = วัดความกว้างจากภาพจริง แล้วใช้เป็นกรอบโดนตี (ไม่กระทบระยะที่มอน "ตีเรา")
@export var hit_box_from_sprite: bool = true
## นับความกว้างของภาพกี่ % เป็นตัวจริง (เหลือขอบไว้เผื่อขน/หาง/ปีกที่ไม่ควรโดนฟัน)
@export_range(0.3, 1.0, 0.05) var hit_box_width_ratio: float = 0.7

# ---------- ★ รอบ 87 — บอสไล่ทั่วสนาม ★ ----------
## บอสล็อกเป้าทันทีที่ผู้เล่นอยู่ในแมพเดียวกัน ไม่ต้องรอเข้าระยะ Detect Range
## (ไม่งั้นบอสจะเดินวนอยู่ในวง Wander Range ของตัวเองทั้งที่ผู้เล่นยืนอยู่อีกฝั่งของสนาม)
@export var boss_arena_aggro: bool = true
## โชว์หลอดเลือดใหญ่กลางจอด้านบนตอนสู้ (แทนหลอดเล็กเหนือหัว)
@export var use_boss_bar: bool = true
...
## ขนาดเอฟเฟกต์ระเบิดบนจอ (สูง) — ภาพชีทใหม่รอบ 82 เตี้ยกว้าง สูง 210 ≈ กว้าง 302
@export var skill_explosion_height: float = 210.0
