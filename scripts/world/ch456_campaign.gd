...
func _ninth_wall() -> void:
	...
	if PlayerState.inventory.count_of(&"gerd_chisel") == 0:
		await _talk([{"name": "ผนังว่าง", "text": "ต้องใช้ค้อนสลักของเกอร์ด (จากเควส รูนดวงที่สี่) — ช่างยักษ์ตาบอดบอกไว้ว่าวันหนึ่งเจ้าจะต้องสลักสิ่งที่นางสลักไม่ได้"}])
		return
	if not PlayerState.has_flag(&"name_left"):
		await _talk([{"name": "ผนังว่าง", "text": "เจ้ายังมีชื่อ... คนมีชื่อสลักบนผนังนี้ไม่ได้ ต้องทิ้งชื่อที่สะพานก่อน (ทิ้งชื่อไว้ที่สะพาน)"}])
		return
	# ---------- พิธี ----------
	_busy = true
	await _talk([
...
func _throne() -> void:
	...
	Events.say("[บัลลังก์ว่าง] เจ้าได้ยินคำว่า «ลูกข้า—» กลับไปบอกเฮล")
	_busy = false
