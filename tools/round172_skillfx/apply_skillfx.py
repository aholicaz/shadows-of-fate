# ★ รอบ 172 ★ ขยายภาพสกิลมอนให้สมกับขนาดตัว (รันซ้ำได้ — เปลี่ยนเฉพาะช่องที่ยังเป็นค่าเดิม)
# ตาราง ค่าเดิม → ค่าใหม่ อยู่ใน fx_table.json ข้างไฟล์นี้ · สำรองไฟล์เดิมที่ _to_delete/ก่อนรอบ172_skillfx/
...
# วิธีใช้: python tools/round172_skillfx/apply_skillfx.py   (ที่รากโปรเจกต์)
import os, sys, re, shutil, json

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = sys.argv[1] if len(sys.argv) > 1 else os.path.abspath(os.path.join(HERE, "..", ".."))
BK = os.path.join(ROOT, "_to_delete", "ก่อนรอบ172_skillfx")
